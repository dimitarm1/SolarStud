...
uses Windows, SysUtils,DateUtils, Classes, Graphics, Forms, Controls, Menus,
  ...
  wwdblook, Wwdbdlg,  AdvCombo, Lucombo, dblucomb, LMDCustomBevelPanel,
...
type
  TMainForm = class(TForm)
    ...
    procedure Label28MouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    ...
    procedure PlannerMaskDatePicker2DaySelect(Sender: TObject;
      SelDate: TDateTime);
    procedure LMDButton16Click(Sender: TObject);
...
uses CHILDWIN, Password, Notshalter, Scan, QReportKlienti,
...
begin
    ...
    MainForm.IniFile.AppDataPath:=AppPath;
    MainForm.IniFile.FilePrefix:=ppCustom;
    MainForm.IniFile.FileName:=AppPath+'SolarStudio1.ini';
    MainForm.IniFile.Open;
    MainForm.IniFile.Update;
    ComPortName:=MainForm.IniFile.ReadString('System','ComPort','COM20') ;
    if ComPortName='COM20' then //this is because KeyExists function is not working
      begin
       MainForm.IniFile.DeleteKey('System','ComPort');
       MainForm.IniFile.WriteString('System','ComPort','COM2');
      MainForm.IniFile.Update;
     end;
...
begin
  AppPath:=ExtractFilePath( Application.ExeName);
  FlashName:=MainForm.IniFile.ReadString('System','FlashMovie','NoFlash') ;
   if FlashName='NoFlash' then //this is because KeyExists function is not working
      begin
       MainForm.IniFile.DeleteKey('System','FlashMovie');
       FlashName:=AppPath+'adimage.swf';
       MainForm.IniFile.WriteString('System','FlashMovie',FlashName);
       MainForm.IniFile.Update;
      end;
    FlashName:=MainForm.IniFile.ReadString('System','FlashMovie',FlashName) ;
    Main2.FlashPlayer.Movie:=FlashName;
    Main2.FlashPlayer.Play;
    MainForm.IniFile.close;
end;
...
//   DATA_COMBO_SELECTED:=Date;
  ...
  Main2.StatusShape1.Color:=MainForm.StatusShape1.Color;
  Main2.StatusShape1.Visible:=MainForm.StatusShape1.Visible;
    Main2.StatusShape2.Color:=MainForm.StatusShape2.Color;
  Main2.StatusShape2.Visible:=MainForm.StatusShape2.Visible;
    Main2.StatusShape3.Color:=MainForm.StatusShape3.Color;
  Main2.StatusShape3.Visible:=MainForm.StatusShape3.Visible;
    Main2.StatusShape4.Color:=MainForm.StatusShape4.Color;
  Main2.StatusShape4.Visible:=MainForm.StatusShape4.Visible;
    Main2.StatusShape5.Color:=MainForm.StatusShape5.Color;
  Main2.StatusShape5.Visible:=MainForm.StatusShape5.Visible;
    Main2.StatusShape6.Color:=MainForm.StatusShape6.Color;
...
// MainForm.Label92.Caption:=IntToHex(IOByte,2);
 ...
 // For demo moide added following:
  ...
  IOResult:=True;
  temp3:=240 div MainForm.Table1.RecordCount;
...
//   if (NewStatus=0)and (CabineTime[SolariumNo]=0) then SolariumNo:=6;
   ...
   case CabineOldStatus[SolariumNo] of
     1..3:
       begin
           case SolariumNo of
            0: MainForm.Kabina1minuti.Caption:='Остават'+IntToStr(CabineTime[0])+'мин.';
            1: MainForm.Kabina2minuti.Caption:='Остават'+IntToStr(CabineTime[1])+'мин.';
            2: MainForm.Kabina3minuti.Caption:='Остават'+IntToStr(CabineTime[2])+'мин.';
            3: MainForm.Kabina4minuti.Caption:='Остават'+IntToStr(CabineTime[3])+'мин.';
            4: MainForm.Kabina5minuti.Caption:='Остават'+IntToStr(CabineTime[4])+'мин.';
            5: MainForm.Kabina6minuti.Caption:='Остават'+IntToStr(CabineTime[5])+'мин.';
           end;
           //added for second form
           case SolariumNo of
            0: Main2.Kabina1minuti.Caption:='Остават'+IntToStr(CabineTime[0])+'мин.';
            1: Main2.Kabina2minuti.Caption:='Остават'+IntToStr(CabineTime[1])+'мин.';
            2: Main2.Kabina3minuti.Caption:='Остават'+IntToStr(CabineTime[2])+'мин.';
            3: Main2.Kabina4minuti.Caption:='Остават'+IntToStr(CabineTime[3])+'мин.';
            4: Main2.Kabina5minuti.Caption:='Остават'+IntToStr(CabineTime[4])+'мин.';
            5: Main2.Kabina6minuti.Caption:='Остават'+IntToStr(CabineTime[5])+'мин.';
           end;
           // end added
           case SolariumNo of
   ...
   If Update then
    begin
     if  CabineOldStatus[SolariumNo]<>0 then
           begin
            case SolariumNo of
            0: MainForm.StatusShape1.Color:=StatusColors[CabineStatus[SolariumNo]];
            1: MainForm.StatusShape2.Color:=StatusColors[CabineStatus[SolariumNo]];
            2: MainForm.StatusShape3.Color:=StatusColors[CabineStatus[SolariumNo]];
            3: MainForm.StatusShape4.Color:=StatusColors[CabineStatus[SolariumNo]];
            4: MainForm.StatusShape5.Color:=StatusColors[CabineStatus[SolariumNo]];
            5: MainForm.StatusShape6.Color:=StatusColors[CabineStatus[SolariumNo]];
            end;
            case SolariumNo of
...
// Label1.Caption:=IntToStr(TimerTime1);
 ...
 if TimerTime1=13 then
  begin
    maxday.Active:=TRUE;
    if dateof(now)<maxday.FieldValues['maxdata'] then
      begin
   ...
   UpdatePageControl(0);
   initFlash();    //Initialize flash movie
  end;
  if TimerTime<3 then
   begin
...
SDELKANOMER:=0;
 with MainForm do
  begin
   ...
   Price:=Q1.FieldValues['CENA'];
   Temp:=Q1.RecNo;
   if IsOK then Price:=Price+0.0000001;
...
if T> Table1.FieldValues['VREME'] then
else  TimeSet:=T;
 FillValues1();
end;
...
// Table1.Locate('SOLARIUM',IntToStr(IndexSol),[loCaseInsensitive]) ;
 ...
 FillValues1();
 if table1.FieldValues['SAOBSHTAVA'] and
   ((table1.FieldValues['LICEVICHAS']<0)or(table1.FieldValues['LAMPICHAS']<0)) then
...
begin
if (PasswordForm.Edit1.Text='43211234') then
          begin
...
Table1.Edit;
Table1.Append;
Table1.FieldValues['CENA']:=0.6;
Table1.FieldValues['VREME']:=5;
Table1.FieldValues['OHLAZDANE']:=2;
Table1.FieldValues['PREDVVREME']:=7;
Table1.FieldValues['OPISANIE1']:='НОВА КАБИНА '+IntToStr(Table1.RecordCount+1);
Table1.FieldValues['MIVREME']:=5;
Table1.FieldValues['SAOBSHTAVA']:=True;
Table1.FieldValues['ADDRESS']:=0;
Table1.FieldValues['MIVREME']:=5;
Table1.FieldValues['LICEVICHAS']:=500;
Table1.FieldValues['LAMPICHAS']:=500;
Table1.Post;
...
begin
 ...
 while (not table1.Eof) do
   begin
     if table1.FieldValues['SOLARIUM']<>table1.RecNo then
      begin
       table1.edit;
       Table1.FieldValues['SOLARIUM']:=Table1.RecNo;
       Table1.Post;
      end;
      Table1.Next;
   end;
 ...
 if not  sol1.TableExists('tablica_ceni') then
  ...
  if q1.RecordCount<60 then
   ...
   For  i1:= 0 to 16  do
   begin
    if i1<table1.RecordCount-1 then table1.RecNo:=i1+1;
    cena1:=table1.FieldValues['CENA'];
    begin
       for i2:= 0 to 30 do
       begin
         Q1.Append;
 ...
 CeniTable.IndexFieldNames:='SOLARIUM';
 CeniTable.Active:=True;
 CeniGrid.Columns.State:=csCustomized;
 CeniGrid.Columns.Delete(0);
 case Solariums of
 2:set2main;
 3:set3main;
 4:set4main;
 ...
 6:set6main;
  ...
  IsDemoMode:=AnsiContainsText(PChar( ExtractFileName( Application.ExeName)),PChar('demo'));
// ScanForm.ShowModal;
end;
...
var
 ...
 i: Byte;
begin
 if Price-(PaidCard+PaidCash+PaidChipCard)<0.02 then
...
{   INIVault1.Open;
   INIVault1.WriteDateTime('systemlog','time-',time);
   INIVault1.WriteString('systemlog','solarium-',(IntToStr(IndexSol)+IntToStr(TimeSet)));
   INIVault1.Update;
   INIVault1.Close; }
   if Retry>21 then
    begin
...
begin
If (AdvPageControl1.ActivePageIndex<3) then
  begin
   if Key = #13 then
    begin
      Key := #0;
   ...
   begin
    ...
    If PasswordForm.ModalResult=mrOK then
        ...
        if ((PasswordForm.Edit1.Text='43211234')or(PasswordForm.Edit1.Text='master1234')) then
          begin
 ...
 begin
  ...
  begin
   ...
   LMDMemo1.Lines.Add('Остава - '+ConvertCurr1(Price-(PaidCard+PaidCash+PaidChipCard)));
    if Price-(PaidCard+PaidCash+PaidChipCard)<0.02 then Label28.Visible:=true;
 end;
...
begin
 ...
 if (AFieldName= 'IME')and (not(AnsiContainsText(QKlienti.SQL.Text, 'IME'))) then
    begin
      QKlienti.Active:=False;
      QKlienti.SQL.SetText(PChar('SELECT * FROM klienti ORDER BY IME DESC'));
    end;
   if (AFieldName = 'NOMER')  then
    begin
      ...
      if AnsiContainsText(QKlienti.SQL.Text, 'DESC')then
       ...
       QKlienti.SQL.SetText(PChar('SELECT * FROM klienti ORDER BY NOMER ASC'));
    end;
    if (AFieldName = 'BALANS') and(not(AnsiContainsText(QKlienti.SQL.Text, 'BALANS')))then
    begin
      QKlienti.Active:=False;
      QKlienti.SQL.SetText(PChar('SELECT * FROM klienti ORDER BY BALANS DESC'));
    end;
    if AnsiContainsText(QKlienti.SQL.Text, 'DESC')then
  ...
  begin
   if Price>0 then
    begin
     ...
     Label72.visible:=True;
  //    Edit8.Visible:=True;
     PaidChipCard:=Price-(PaidCard+PaidCash);
     if PaidChipCard>Card.Balans then PaidChipCard:=Card.Balans;
     FmtStr(Result1,'%4.2f',[PaidChipCard]);Edit8.Text:=Result1;
     Ostatak:= (Card.Balans - PaidChipCard)/Table1.FieldValues['CENA'];
   ...
   begin
    ...
    if ((not (Card.ErrCounter>2)) or (Card.ConStatus=0)) then exit
     ...
     begin
       ...
       SLE4442WriteCardInfo();
       SLE4442ShowCardInfo();
       if ((Card.CardNomer<0)and (KartaNomer>0)) then //successful cleaning - return deposit
        begin
...
begin  //Зареждане
 if Card.ErrCounter>2 then
  begin
   if Card.CardNomer<0 then  // new card
    begin
     Data:=Card.PIN; //'ffffff';
     SLE4442Submit();
     SLE4442ReadCardInfo();
     if Card.ErrCounter>5 then
      begin
   ...
   if (Card.CardNomer>0) and (Card.ClientNomer <> QKlienti.FieldValues['NOMER']) then
    begin
      Application.MessageBox(PChar('Картата е на друг клиент и не е нулирана!'),PChar('Warning'),MB_OK);
       SLE4442ReadCardInfo();SLE4442ShowCardInfo();
       exit;
    end;
   NewCard:=(Card.StudioNomer=0);
...
procedure TMainForm.LMDButton11Click(Sender: TObject);
var
  maxnomer: integer;
  nomer: integer;
  L: Boolean;
...
if Timer2Time>length('www.stefano-sport.com') then
  begin
...
procedure TMainForm.Label28MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  Label28.Top:=Label28.Top+3;
  Label28Click(sender) ;
end;

procedure TMainForm.Label28MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
   Label28.Top:=Label28.Top-3;
end;
...
procedure TMainForm.LMDButton16Click(Sender: TObject);
var
  i1: Integer;
begin
   CeniTable.First;
   While (not CeniTable.Eof) do
     begin
       CeniTable.Edit;
       CeniTable.FieldValues['CENA']:=CeniTable.FieldValues['MINUTA']*Table1.FieldValues['CENA'];
       CeniTable.Post;
       CeniTable.Next;
     end;
end;
