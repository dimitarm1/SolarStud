...
uses Windows, SysUtils,DateUtils, Classes, Graphics, Forms, Controls, Menus,
  ...
  ActnList, ToolWin, ImgList, DB, DBCtrls, Grids, Gauges,  sqlite3conn,
  DBGrids, ValEdit, ImageWin, setformdata, Variants,   Math, sqldb, EditBtn,
  StrUtils, MainForm2, inifiles, TAGraph;
...
type
  TMainForm = class(TForm)
    ...
    Label8: TLabel;
    Label9: TLabel;
    sol1: TSQLite3Connection;
    SOLARIUMI: TSQLQuery;
    Table3: TSQLQuery;
    AdvPageControl1: TPageControl;
    AdvTabSheet1: TTabSheet;
    AdvTabSheet2: TTabSheet;
    AdvTabSheet3: TTabSheet;
    AdvTabSheet4: TTabSheet;
    AdvTabSheet5: TTabSheet;
    AdvTabSheet6: TTabSheet;
    AdvTabSheet7: TTabSheet;
    AdvTabSheet8: TTabSheet;
    AdvTabSheet9: TTabSheet;
    AdvTabSheet10: TTabSheet;
    AdvTabSheet11: TTabSheet;
    AdvTabSheet12: TTabSheet;
    AdvTabSheet13: TTabSheet;
    AdvTabSheet14: TTabSheet;
    AdvTabSheet15: TTabSheet;
    AdvTabSheet16: TTabSheet;
    AdvTabSheet17: TTabSheet;
    Label96: TLabel;
    LMDDBCheckBox1: TDBCheckBox;
    Label30: TLabel;
    Label56: TLabel;
    Label97: TLabel;
    Image52: TImage;
    ...
    Label129: TLabel;
    Label130: TLabel;
    AdvComboBox1: TComboBox;
    Image55: TImage;
    Label6: TLabel;
    KasaGrid: TDBGrid;
    Sdelka: TSQLQuery;
    PlashtaniaTable: TSQLQuery;
    ...
    Kabina32: TLabel;
    Image33: TImage;
    PlannerMaskDatePicker1: TDateEdit;
    PlannerMaskDatePicker2: TDateEdit;
    Label147: TLabel;
    Label148: TLabel;
    Label149: TLabel;
    Label58: TLabel;
    ...
    Label134: TLabel;
    Label135: TLabel;
    AdvTabSheet19: TTabSheet;
    wwDBGrid5: TDBGrid;
    NovKlientButton: TButton;
    Label136: TLabel;
    Label137: TLabel;
    Label138: TLabel;
    Label139: TLabel;
    ...
    Image21: TImage;
    Image128: TImage;
    LMDImageList3: TImageList;
    LMDImageList4: TImageList;
    LMDLImage5: TImage;
    LMDLImage6: TImage;
    LMDLImage7: TImage;
    LMDLImage8: TImage;
    LMDLImage9: TImage;
    LMDImageList5: TImageList;
    LMDLImage11: TImage;
    LMDLImage12: TImage;
    LMDLImage14: TImage;
    LMDLImage15: TImage;
    ...
    LMDLImage27: TImage;
    DBText15: TDBText;
    LMDSimpleLabel8: TLabel;
    DBText16: TDBText;
    DBText17: TDBText;
    Timer3: TTimer;
    maxday: TSQLQuery;
    Plashtania: TSQLQuery;
    Label89: TLabel;
    Label145: TLabel;
    Label159: TLabel;
    PosEdit: TDBEdit;
    ...
    LMDLImage146: TImage;
    LMDLImage147: TImage;
    NuliraneChipCartaButton: TButton;
    Timer4: TTimer;
    DataSource17: TDataSource;
    QCeni: TSQLQuery;
    Label28: TLabel;
    StokiteTab: TPageControl;
    AdvTabSheet20: TTabSheet;
    AdvTabSheet21: TTabSheet;
    AdvTabSheet22: TTabSheet;
    wwDBGrid4: TDBGrid;
    wwDBGrid3: TDBGrid;
    Label124: TLabel;
    ...
    Label112: TLabel;
    RangCombo: TDBComboBox;
    wwDBGrid13: TDBGrid;
    LMDLImage22: TImage;
    Label163: TLabel;
    FirmaCombo: TDBLookupComboBox;
    AdvTabSheet23: TTabSheet;
    Image7: TImage;
    LMDLImage23: TImage;
    Label164: TLabel;
    LMDLImage150: TImage;
    ...
    procedure RzBmpButton1MouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure RzBmpButton1MouseUpton1MouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    ...
    procedure Planner2HeaderDblClick(Sender: TObject; SectionIndex: Integer);
...
type TSolariumTime = packed record // dcb
     Pretime: Integer;
     MainTime: Integer;
     CoolTime: Integer;
end;
...
function IntToStr2(value:variant):string;
begin
 if varType(Value) <> varNull then result:=IntToStr(value)
 else result:='0';
end;
 ...
 _Q.DatabaseName:='sol1'; 
  ...
  begin
    ...
    // except  Application.MessageBox('Грешка в базата данни - Възстановете от архивно копие!', 'ГРЕШКА', MB_OK);
     ...
     if stoki.FieldDefs.IndexOf('STOKATIP')<0 then  begin
     ...
     if stoki.FieldDefs.IndexOf('STOKACENACARD')<0 then  begin
       Stoki.RestructureFieldDefs.Add('STOKACENACARD',aftCurrency,1,FALSE);
       Stoki.RestructureTable;
       Stoki.Close;
     end;
     ...
     while not Stoki.Eof do
      begin
        if (Stoki.FieldValues['STOKAKOD'] >0) and( (Stoki.FieldValues['STOKATIP']='' )
        ...
        if (Stoki.FieldValues['STOKAKOD'] <0) and  (Stoki.FieldValues['POSESHTENIA'] =-1) and
        ...
        if (Stoki.FieldValues['STOKAKOD'] <0) and  (not(Stoki.FieldValues['POSESHTENIA'] =-1)) and
     ...
     if not  sol1.TableExists('tablica_ceni') then
      begin
       _Q.SQL.Text:='create table TABLICA_CENI ( SOLARIUM integer default 0, '+
      ...
      if _Q.RecordCount<60 then
       ...
       For  i1:= 0 to 16  do
       begin
        if i1<Solariumi.RecordCount-1 then Solariumi.RecNo:=i1+1;
      ...
      if (Internet.Active=FALSE) then Internet.Active:=TRUE;

      if (Internet.FieldByName('DISCOUNT_PERCENT').DataType=ftInteger) then begin
        Internet.Close;
      ...
      if (KARTICHIP.Active=FALSE) then KARTICHIP.Active:=TRUE;
      if (KARTICHIP.FieldByName('DISCOUNT').DataType=ftInteger) then begin
        KARTICHIP.Close;
      ...
      if (PLASHTANIA.Active=FALSE) then PLASHTANIA.Active:=TRUE;
      if (PLASHTANIA.FieldByName('DISCOUNT').DataType in [ftSmallint,ftInteger]) then begin
        ...
        _Q.SQL.Text:='alter table PLASHTANIA modify (DISCOUNT  FLOAT, SUMABROI CURRENCY)';
        _Q.ExecSQL;// .Active:=true;
        _Q.Active:=FALSE;
...
begin
    ...
    if ComPortName='COM20' then //this is because KeyExists function is not working
      begin
   ...
   PlannerStep := IniFile.ReadString('System','PlannerStep','NoValue') ;
    if PlannerStep = 'NoValue' then //this is because KeyExists function is not working
      begin
       ...
       IniFile.WriteString ('System','PlannerStep',IntToStr(MainForm.Planner1.Display.DisplayUnit));
       IniFile.UpdateFile;
      end;
    MainForm.Planner1.Display.DisplayUnit := StrToInt(IniFile.ReadString('System','PlannerStep',IntToStr(MainForm.Planner1.Display.DisplayUnit)) );
    MainForm.Planner2.Display.DisplayUnit := MainForm.Planner1.Display.DisplayUnit;
    MainForm.Planner3.Display.DisplayUnit := MainForm.Planner1.Display.DisplayUnit;

    DemoString := IniFile.ReadString('System','Demo','NoValue') ;
    if DemoString = 'NoValue' then //this is because KeyExists function is not working
      begin
   ...
   counter:=0;
    While Counter<5 do
     begin
      ...
      if (hDevice = INVALID_HANDLE_VALUE) then Counter :=Counter+1
       else Counter:=6;    // did we succeed?
    end;
...
//    DayTotal.ParamByName('ORDERPARAM').AsString:=' STOKA ';
    ...
    begin
     ...
     if i=0 then
      begin
        ...
        else Imagesol1.Picture:=NulImage.Picture;
     ...
     begin
     Kabina1minuti.Caption:=SOLARIUMI.FieldByName('OPISANIE3').AsString;
     Kabina1cena.Caption  :=SOLARIUMI.FieldByName('OPISANIE4').AsString;
     ...
     begin
      Kabina2minuti.Caption:=SOLARIUMI.FieldByName('OPISANIE3').AsString;
      Kabina2cena.Caption  :=SOLARIUMI.FieldByName('OPISANIE4').AsString;
     ...
     begin
      Kabina3minuti.Caption:=SOLARIUMI.FieldByName('OPISANIE3').AsString;
      Kabina3cena.Caption  :=SOLARIUMI.FieldByName('OPISANIE4').AsString;
     ...
     begin
      Kabina4minuti.Caption:=SOLARIUMI.FieldByName('OPISANIE3').AsString;
      Kabina4cena.Caption  :=SOLARIUMI.FieldByName('OPISANIE4').AsString;
     ...
     begin
      Kabina5minuti.Caption:=SOLARIUMI.FieldByName('OPISANIE3').AsString;
      Kabina5cena.Caption  :=SOLARIUMI.FieldByName('OPISANIE4').AsString;
  ...
  //   else Imagesol6.Picture:=Imagepress1.Picture;
     ...
     begin
       Kabina6minuti.Caption:=SOLARIUMI.FieldByName('OPISANIE3').AsString;
       Kabina6cena.Caption  :=SOLARIUMI.FieldByName('OPISANIE4').AsString;
...
begin
 ...
 if FromTable then
  begin
   ...
   if Chanel>15 then IsDemoMode:=True else IsDemoMode:=False;
   if IsDemoMode then
    begin
...
// MainForm.Label92.Caption:=IntToHex(IOByte,2);
 ...
 if IOResult then
  begin
   if IOCount=0 then NewStatus:=4 //disconected
     else NewStatus:=IOByte div 64;

//   if (NewStatus=0)and (CabineTime[SolariumNo]=0) then SolariumNo:=6;
   if (NewStatus=CabineStatus[SolariumNo]) and  (NewStatus<>CabineOldStatus[SolariumNo]) then
     begin Update:= true;
     ...
     begin
      Update:=false;
      CabineStatusCounter[solariumNo]:=CabineStatusCounter[solariumNo]+1;
     end;
   if NewStatus=CabineStatus[SolariumNo] then  CabineOldStatus[SolariumNo]:= CabineStatus[SolariumNo];
   ...
   if (TimerTime1>30) and (TimerTime1<50)then update:=True;
   case CabineOldStatus[SolariumNo] of
     1..3:
       begin
           case SolariumNo of          //'Остават'       'мин.'
            0: MainForm.Kabina1minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[0])+' '+GetMessage('M81');
            1: MainForm.Kabina2minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[1])+' '+GetMessage('M81');
            2: MainForm.Kabina3minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[2])+' '+GetMessage('M81');
            3: MainForm.Kabina4minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[3])+' '+GetMessage('M81');
            4: MainForm.Kabina5minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[4])+' '+GetMessage('M81');
            5: MainForm.Kabina6minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[5])+' '+GetMessage('M81');
           end;
           //added for second form
           case SolariumNo of
            0: Main2.Kabina1minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[0])+' '+GetMessage('M81');
            1: Main2.Kabina2minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[1])+' '+GetMessage('M81');
            2: Main2.Kabina3minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[2])+' '+GetMessage('M81');
            3: Main2.Kabina4minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[3])+' '+GetMessage('M81');
            4: Main2.Kabina5minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[4])+' '+GetMessage('M81');
            5: Main2.Kabina6minuti.Caption:=GetMessage('M80')+' '+IntToStr(CabineTime[5])+' '+GetMessage('M81');
           end;
           // end added
           case SolariumNo of
    ...
    case CabineOldStatus[SolariumNo] of
     1:
       begin
         if CabineStartTime[SolariumNo]=0 then  CabineStartTime[SolariumNo]:=now;
       end;
     0:
       begin
         if CabineStatusCounter[SolariumNo]>10 then   CabineStartTime[SolariumNo]:=0;
         if (CabineStatusCounter[SolariumNo]>1000)  then  CabineSetTime[SolariumNo]:=0;
       end
   end;
  end
  else
 ...
 //  if  (SolariumNo=0) then MainForm.Kabina1Cena.Caption:=IntToStr(LastTime[SolariumNo])+' err:'+IntToStr(CabineErrCounter[SolariumNo]);
  if ((CabineOldStatus[SolariumNo]=0)or(CabineOldStatus[SolariumNo]=2)) and (LastTime[SolariumNo]>2) then
    CabineErrCounter[SolariumNo]:= CabineErrCounter[SolariumNo]+1 else CabineErrCounter[SolariumNo]:=0;
  if   CabineErrCounter[SolariumNo]>4 then
   begin
   CabineErrCounter[SolariumNo]:= 0;
    MainForm.Timer1.Enabled:=False;
    StrBuff:='';//('Солариум '+IntToStr(SolariumNo+1)+' ненадейно се нулрира по време на работа! Да го заредя ли отново?');
   ...
   If Update then
    begin
     if  CabineOldStatus[SolariumNo]<>0 then
           begin
            case SolariumNo of
            0: MainForm.StatusShape1.Brush.Color:=StatusColors[CabineStatus[SolariumNo]];
            1: MainForm.StatusShape2.Brush.Color:=StatusColors[CabineStatus[SolariumNo]];
            2: MainForm.StatusShape3.Brush.Color:=StatusColors[CabineStatus[SolariumNo]];
            3: MainForm.StatusShape4.Brush.Color:=StatusColors[CabineStatus[SolariumNo]];
            4: MainForm.StatusShape5.Brush.Color:=StatusColors[CabineStatus[SolariumNo]];
            5: MainForm.StatusShape6.Brush.Color:=StatusColors[CabineStatus[SolariumNo]];
            end;
            case SolariumNo of
...
// if (f1=nil) or (f2=nil) then show_message:=False;
 ...
 if show_message then
  begin
  Backuped:=True;
   if CopyStatus then Application.MessageBox(PChar(GetMessage('M4')),PChar(GetMessage('M6')),MB_OK)
   // if CopyStatus then Application.MessageBox(PChar('Архивиране на данните... успешно!'),PChar('Архивиране'),MB_OK)
    else  Application.MessageBox(PChar(GetMessage('M5')),PChar(GetMessage('M6')),MB_OK);
...
//    Timer1.Enabled:=True;
 ...
 if TimerTime1=13 then
  begin
   ...
   initFlash();
   Main2.Visible:=True;
   Main2.Show; MainForm.Visible:=true;
   MainForm.Show;
   MainForm.SetFocus;
  end;
  if TimerTime1 mod 20 = 18 then begin
    dateofd := dateof(now);
    if (dateofd < maxday.FieldValues['maxdata']) or ((dateofd > 40330 + 100)
      and FileExists(Application.ExeName+'.ini')) then  begin
       Timer1.Enabled := false;
 ...
 if (TimerTime1<13)and (TimerTime1>3)then
   begin
     ...
     if IOCheck>0 then IOCount:=1 else IOCount:=0;

     if ( (not SOLARIUMI.Eof)) then
         begin
          case (TimerTime1-3) of
           1:  begin
                  if FileExists(ImageName) then Image34.Picture.LoadFromFile(ImageName);
                  Label75.Caption:=StatusChar[IOCount];
                   No1.Caption:=SOLARIUMI.FieldByName('OPISANIE2').AsString;
               end;
           2: begin
                  if FileExists(ImageName) then Image35.Picture.LoadFromFile(ImageName);
               Label76.Caption:=StatusChar[IOCount];
                No2.Caption:=SOLARIUMI.FieldByName('OPISANIE2').AsString;
               end;
           3: begin
                  if FileExists(ImageName) then Image36.Picture.LoadFromFile(ImageName);
                Label77.Caption:=StatusChar[IOCount];
                 No3.Caption:=SOLARIUMI.FieldByName('OPISANIE2').AsString;
               end;
           4: Begin
                  if FileExists(ImageName) then  Image37.Picture.LoadFromFile(ImageName);
                Label78.Caption:=StatusChar[IOCount];
                 No4.Caption:=SOLARIUMI.FieldByName('OPISANIE2').AsString;
               end;
           5: begin
                  if FileExists(ImageName) then  Image38.Picture.LoadFromFile(ImageName);
                Label79.Caption:=StatusChar[IOCount];
                 No5.Caption:=SOLARIUMI.FieldByName('OPISANIE2').AsString;
               end;
           6: begin
                  if FileExists(ImageName) then Image39.Picture.LoadFromFile(ImageName);
                Label80.Caption:=StatusChar[IOCount];
                 No6.Caption:=SOLARIUMI.FieldByName('OPISANIE2').AsString;
               end;
           end;
         end
    ...
    if TimerTime1=9 then TimerTime1:=12
   end;
...
T: Integer;
begin
TimeSet:=TimeSet-SOLARIUMI.FieldValues['STAPKA']*SOLARIUMI.FieldValues['KOEFICIENT'];
if TimeSet<SOLARIUMI.FieldByName('MIVREME').AsInteger then TimeSet:=SOLARIUMI.FieldByName('MIVREME').AsInteger ;
FillValues1();
end;
...
T: Integer;
begin
  T:= TimeSet+SOLARIUMI.FieldValues['STAPKA']*SOLARIUMI.FieldValues['KOEFICIENT'];
  if T <= SOLARIUMI.FieldValues['VREME'] then
    TimeSet:=T;
  FillValues1();
end;
...
procedure LocateSolarium;
 ...
 with MainForm do
  ...
  if SOLARIUMI.FieldValues['SAOBSHTAVA'] and
    ((SOLARIUMI.FieldValues['LICEVICHAS']<0)or(SOLARIUMI.FieldValues['LAMPICHAS']<0)) then
       Application.MessageBox('Лампите са за смяна!!', 'ПРЕДУПРЕЖДЕНИЕ', MB_OK);
...
begin
  ...
  if (Sender=Imagepress1) or (Sender=ImageSol1) or (Sender=StatusShape1) then  IndexSol:=1;
  if (Sender=Imagepress2) or (Sender=ImageSol2) or (Sender=StatusShape2) then  IndexSol:=2;
  if (Sender=Imagepress3) or (Sender=ImageSol3) or (Sender=StatusShape3) then  IndexSol:=3;
  if (Sender=Imagepress4) or (Sender=ImageSol4) or (Sender=StatusShape4) then  IndexSol:=4;
  if (Sender=Imagepress5) or (Sender=ImageSol5) or (Sender=StatusShape5) then  IndexSol:=5;
  if (Sender=Imagepress6) or (Sender=ImageSol6) or (Sender=StatusShape6) then  IndexSol:=6;
  if Button=mbRight then
    begin
    MainForm.Timer1.Enabled:=False;
     NotshalterForm.Label1.Caption:='КАБИНА '+IntToStr(IndexSol);
     NotshalterForm.ShowModal;
     if (NotshalterForm.ModalResult=mrOk)  and (CabineStatus[IndexSol-1]=3) then SendCmd(1);
     if NotshalterForm.ModalResult=mrIgnore then SendCmd(2);
     if (NotshalterForm.ModalResult=mrAbort) and (CabineStatus[IndexSol-1]=0)  then SendCmd(3);
...
SOLARIUMI.FieldValues['OPISANIE1']:='New Solarium '+IntToStr(SOLARIUMI.RecordCount+1);
...
// OpenTables;
 ...
 6:set6main;
  ...
  StatusColors[0]:=clGray; StatusColors[1]:=clRed;StatusColors[2]:=clBlue;
...
if sender1 = sender then;
 ...
 _Q.ReadOnly:=False;
 _Q.RequestLive:=True;
 _Q.SQL.SetText(PChar('SELECT * FROM kartichip WHERE KLIENTNOMER = '+IntToStr(CardNomer)));
 _Q.Active:=true;
 ...
 if Price-(PaidCard+ToBePaidCash+PaidChipCard)<0.03 then
  if PaidChipCard>0 then
   begin
    ...
    if (Card.ErrCounter>2) then
     ...
     if CardNomer=0 then
     begin
      Application.MessageBox(PChar(GetMessage('M11')),PChar('Warning'),MB_OK);
  ...
  begin
   ...
   if Retry>21 then
    begin
    if not ((Plashtania.Locate('RecordID',  CabineRecord[IndexSol],[])) and (Plashtania.FieldValues['DATA']=Date)
      and (Plashtania.FieldValues['BROI']=CabineSetTime[ (IndexSol-1)]) and (MinutesBetween(StrToTime(Plashtania.FieldValues['CHAS']),(Time))<5))then
    begin
     PosSelected:=0;
     ParvaKartaPlatena:=0;
     ParvaKartaPlateno:=0;
...
//     if (PaidCash>0) and (VipDiscount+DiscountPrize>0) then
     ...
     if (_Q.RecordCount>0) and (not (IsChipCard)) and (CardNomer>0) then
       begin
        _Q.Edit;
        _Q.FieldValues['SUMA']:= _Q.FieldValues['SUMA']-(PaidChipCard);//-(PaidChipCard*DiscountPrize/100));
        _Q.Post;
       end
     else
      if (PaidChipCard>0) and not IsChipCard then
       begin
     ...
     if (PaidChipCard > 0)  then
       begin
        ...
        Plashtania.FieldValues['KLIENTNOMER']:=QKlienti.FieldValues['nomer2'];
       end
       else
        if (CardNomer>0) then
        begin
     ...
     if not _Q.IsEmpty and (PaidCash>0)then
      begin
       _Q.Edit;
       _Q.FieldValues['COUNTER']:= KARTICHIP.FieldValues['COUNTER']+1;
       _Q.Post;
      end
     else
       begin
     ...
     SOLARIUMI.Edit;
     SOLARIUMI.FieldValues['LAMPICHAS']:=SOLARIUMI.FieldValues['LAMPICHAS']-(TimeSet/60);
     SOLARIUMI.FieldValues['LICEVICHAS']:=SOLARIUMI.FieldValues['LICEVICHAS']-(TimeSet/60);
     SOLARIUMI.Post;
     end;
     AdvPageControl1.ActivePage:=AdvTabsheet2;
     UpdatePageControl(1);
     ...
     begin
        ...
        if PaidChipCard>0 then
         ...
         SendCmd(2); //Stop
        Application.MessageBox(PChar(GetMessage('M12')), PChar(GetMessage('M13')), MB_OK);
...
begin
   ...
   if (Key = #10)or (Key = #13) then   begin
     ...
     if (rightstr(KeyBuff,1)='?')  then //This is card number entry
       begin
        try
          len:= Length(KeyBuff);
          CardNomer:=StrToInt(rightstr(leftstr(KeyBuff,len-1),len-1));
          Timer3.Enabled:=true;
        except;
        end;
       end
 ...
 If (AdvPageControl1.ActivePageIndex=1) then
  begin
   ...
   begin
    if PasswordForm.visible=true then PasswordForm.visible:=false;
    PasswordForm.ShowModal;
    If PasswordForm.ModalResult=mrOK then
        ...
        if ((PasswordForm.Edit1.Text = MainForm.personal1.FieldValues['parola'] )or(PasswordForm.Edit1.Text='master1234')) then
          begin
...
begin
 DayTotal.ParamByName('DATA_COMBO_SELECTED').AsDate:=StrToDate( AnsiReplaceText(DBLUCombo1.Text,' г.',''));
 DayTotal.Active:=False;
 DayTotal.Active:=True;
end;
...
begin
  case StokiteTab.ActivePageIndex of
     0: KartiAsStokiMenu.Popup(DobaviStokaBtn.Left+Round(DobaviStokaBtn.Width/2),
...
if (TimerTime1>10)and ((AdvPageControl1.ActivePageIndex=3) or (AdvPageControl1.ActivePageIndex=15)) then
 begin
    DBText7.Clear;
    i:=Qklienti.RecNo;
    Qklienti.First;
...
begin
 ...
 USLUGITE.Active:=False; USLUGITE.Active:=TRUE;
 STOKITE.Active:=false; STOKITE.Active:=True;
 SDELKA.active:=false;
...
//            STOKI.FieldValues['STOKAIME']:=GetMessage('M68')+' '+ IntToStr(Round(Suma))+' '+GetMessage('M20');
          ...
          Application.MessageBox(PChar('Вече има създаден депозит'),PChar(''));
      end;
           1: //Клубни Карти
      begin
        ...
        STOKI.FieldValues['STOKAIME']:='Нова Клубна карта'+IntTostr(mincode-1);
        STOKI.FieldValues['STOKACENA']:=0;
        STOKI.FieldValues['STOKANASKLAD']:=0;
        STOKI.FieldValues['STOKATIP']:='K';
        STOKI.Post;
      ...
      begin
        ...
        STOKI.Append;
        STOKI.FieldValues['STOKAKOD']:=mincode-1;
        STOKI.FieldValues['STOKAIME']:='Нова Чип Карта'+IntTostr(mincode-1);
        STOKI.FieldValues['STOKACENA']:=0;
        STOKI.FieldValues['STOKANASKLAD']:=0;
...
begin
 ...
 if STOKI.Modified then
   begin
     Application.MessageBox(PChar('Внимание!'),PChar('Записът се редактира и не може да бъде изтрит!'),MB_OK);
     exit; //-->
   end;
 ...
 if Application.MessageBox(PChar(Message1),PChar(GetMessage('M17')),MB_YESNO)=6 then
...
if (Sender=nil) or (AdvPageControl1.ActivePageIndex=2)then
 begin
  ...
  if AnsiPos('"'+IntToStr(FIRMITE.FieldValues['NOMER'])+'"',QKLIENTI.SQL.Text)<1 then
    begin
     QKLIENTI.Active:=False;
...
begin
  ...
  //Картата ще бъде изчистена!?
   ...
   begin
    ...
    Application.MessageBox(PChar(GetMessage('M76')),PChar('Warning'),MB_OK);
...
begin
 ...
 Table3.FieldValues['IME']:=GetMessage('M66')+IntTostr(maxnomer+1);     //'Нов клиент №'
 Table3.Post;
 Table3.Last;
 QKlienti.Active:=False;
 if not ShowAllKlientsCb.Checked then SQLText := ' WHERE NOMER >-1 ';
 ...
 begin
   if QKLIENTI.State in [dsInsert,dsEdit] then QKLIENTI.Post;
...
begin
 ...
 case index of
  ...
  begin
      MainForm.QStatistika.SQL.Text:='SELECT SUM(STOKA) AS STOKA1, COUNT (*)'+
  'AS BROI1, MIN (SOLARIUM) AS SOLARIUM1, CAST(MIN (SOLARIUM) as STRING) AS WEEKDAY FROM PLASHTANIA WHERE'+
    '(SOLARIUM > 0) AND (DATA BETWEEN "' +MainForm.PlannerMaskDatePicker1.Text +
   '" AND "'+MainForm.PlannerMaskDatePicker2.Text+'")';
     MainForm.Chart1.BottomAxis.Labels:=False;
    end;
  ...
  'AS BROI1,SOLARIUM  AS SOLARIUM1,  CAST(SUM (STOKA) as STRING) AS WEEKDAY FROM PLASHTANIA WHERE'+
    '(SOLARIUM > 0) AND (DATA BETWEEN "' +MainForm.PlannerMaskDatePicker1.Text +
   '" AND "'+MainForm.PlannerMaskDatePicker2.Text+'") GROUP by SOLARIUM';
     MainForm.Chart1.BottomAxis.Labels:=False;
    end;
  ...
  begin
  MainForm.QStatistika.SQL.Text:='SELECT ROUND (SUM(CAST(BROI as float)/60) ,2) '+
  'AS BROI1, MIN (SOLARIUM) AS SOLARIUM1,  CAST(MIN (SOLARIUM) as STRING) AS WEEKDAY FROM PLASHTANIA WHERE'+
  '(SOLARIUM > 0) AND (PLASHTANIA.DATA BETWEEN "' +MainForm.PlannerMaskDatePicker1.Text +
  '" AND "'+MainForm.PlannerMaskDatePicker2.Text+'")';
    MainForm.Chart1.BottomAxis.Labels:=False;
    end;
  ...
  '(SELECT STOKAKOD, STOKAIME AS WEEKDAY FROM STOKI ) S ON P.STOKA = S.STOKAKOD WHERE'+
    '(STOKA > 0) AND (DATA BETWEEN "' +MainForm.PlannerMaskDatePicker1.Text +
   '" AND "'+MainForm.PlannerMaskDatePicker2.Text+'") GROUP BY STOKA, WEEKDAY ';
     MainForm.Chart1.BottomAxis.Labels:=False;
    end;
  ...
  'AS BROI1, STOKA AS SOLARIUM1, S.WEEKDAY FROM PLASHTANIA P LEFT OUTER JOIN '+
  '(SELECT STOKAKOD, STOKAIME AS WEEKDAY FROM STOKI ) S ON P.STOKA = S.STOKAKOD WHERE'+
    '(STOKA <-1) AND (DATA BETWEEN "' +MainForm.PlannerMaskDatePicker1.Text +
   '" AND "'+MainForm.PlannerMaskDatePicker2.Text+'") GROUP BY STOKA, WEEKDAY ';
     MainForm.Chart1.BottomAxis.Labels:=False;
    end;
  10://Работа на солариумите по ден от периода
  begin
        MainForm.QStatistika.SQL.Text:='SELECT DATA, ROUND (SUM(CAST(BROI as float)/60) ,2) AS BROI1'+
    ', (SOLARIUM - SOLARIUM) AS SOLARIUM1,  TOSTRING(DATA,"DD/M") AS WEEKDAY FROM PLASHTANIA WHERE '
     +'(SOLARIUM > 0) AND (DATA BETWEEN "'+MainForm.PlannerMaskDatePicker1.Text +
     '" AND "'+MainForm.PlannerMaskDatePicker2.Text+'") GROUP BY  WEEKDAY, DATA, SOLARIUM1 order by data';
     MainForm.Chart1.BottomAxis.Labels:=True;
    end;
  11://Работа на солариумите по ден от периода по солариум
  begin
        MainForm.QStatistika.SQL.Text:='SELECT  ROUND (SUM(CAST(BROI as float)/60) ,2) AS BROI1'+
    ', SOLARIUM AS SOLARIUM1,  TOSTRING(DATA,"DD/M") AS WEEKDAY FROM PLASHTANIA WHERE '
     +'(SOLARIUM > 0) AND (DATA BETWEEN "'+MainForm.PlannerMaskDatePicker1.Text +
     '" AND "'+MainForm.PlannerMaskDatePicker2.Text+'") group by data, solarium';
     MainForm.Chart1.BottomAxis.Labels:=True;
    end;
  12://Работа на солариумите по ден от седмица общо в лева
      begin
       if MainForm.PlannerMaskDatePicker1.Date>=MainForm.PlannerMaskDatePicker2.Date then MainForm.PlannerMaskDatePicker2.Date:=MainForm.PlannerMaskDatePicker1.Date+7;
       MainForm.PlannerMaskDatePicker2.Date:=MainForm.PlannerMaskDatePicker2.Date+
       (DayOfWeek(MainForm.PlannerMaskDatePicker2.Date)-DayOfWeek(MainForm.PlannerMaskDatePicker1.Date));
         MainForm.QStatistika.SQL.Text:='SELECT   CAST (SUM (SUMABROI) AS CURRENCY) '+
    'AS BROI1, CAST(TOSTRING(DATA,"DW") AS INTEGER) AS SOLARIUM1, TOSTRING(DATA,"DW") AS WEEKDAY FROM PLASHTANIA WHERE '
     +'(SOLARIUM > 0) AND (DATA BETWEEN "'+MainForm.PlannerMaskDatePicker1.Text +
     '" AND "'+MainForm.PlannerMaskDatePicker2.Text+'") group by wEEKDAY, solarium1';
     MainForm.Chart1.BottomAxis.Labels:=True;
    end;
  ...
  16://Работа на солариумите по ден от седмицата по солариум в часове
    ...
    'AS BROI1, SOLARIUM AS SOLARIUM1,  TOSTRING(DATA,"DW") AS WEEKDAY FROM PLASHTANIA WHERE '
     +'(SOLARIUM > 0) AND (DATA BETWEEN "'+MainForm.PlannerMaskDatePicker1.Text +
     '" AND "'+MainForm.PlannerMaskDatePicker2.Text+'") group by wEEKDAY, solarium';
     MainForm.Chart1.BottomAxis.Labels:=True;
    end;
  ...
  'AS BROI1,SOLARIUM AS SOLARIUM1,  TOSTRING(CHAS,"HH24") AS WEEKDAY FROM PLASHTANIA WHERE'+
    '(SOLARIUM > 0) AND (DATA BETWEEN "' +MainForm.PlannerMaskDatePicker1.Text +
   '" AND "'+MainForm.PlannerMaskDatePicker2.Text+'") group by wEEKDAY , solarium ORDER BY WEEKDAY';
    MainForm.Chart1.BottomAxis.Labels:=True;
    end;
  19://Работа на солариумите по месеци
  begin
     if MainForm.PlannerMaskDatePicker1.Date>MainForm.PlannerMaskDatePicker2.Date then MainForm.PlannerMaskDatePicker2.Date:=MainForm.PlannerMaskDatePicker1.Date;
 //      MainForm.PlannerMaskDatePicker2.Date:=MainForm.PlannerMaskDatePicker2.Date-
...
if not QKLIENTI.IsEmpty then
 begin
  ...
  if QKLIENTI.FieldValues['NOMER2'] = null then
   ...
   QKLIENTI.First;
   for i := 0 to QKLIENTI.RecordCount - 1 do begin
     if (Nomer<= QKLIENTI.FieldValues['NOMER2'])then Nomer:=QKLIENTI.FieldValues['NOMER2'];
     QKLIENTI.Next;
   end;
   Nomer:=Nomer+1;
   sol1.StartTransaction;
   try
    ...
    QKLIENTI.FieldValues['IME']:='Нов Служителужител '+ inttostr(Nomer);
    QKLIENTI.Post;
    sol1.Commit();
   except
      sol1.Rollback;
 ...
 else Application.MessageBox(PChar('Избраната фирма няма нито една карта!'),PChar(''))
end;
...
begin
   DayTotal.Last;
   if PasswordForm.ModalResult=MROK then DBLUCombo1.Visible:=true
   else     DBLUCombo1.Visible:=false;
end;

procedure TMainForm.PaymentOKLabelMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
if OpenDialog1.Files.Count>0 then
 begin
  ...
  CopyStatus:=CopyFIle(PChar(FileName1),PChar(FileName2),False);
  if CopyStatus then Application.MessageBox(PChar('Разархивиране ... успешно!'),PChar('Разархивиране'),MB_OK)
    else  Application.MessageBox(PChar('Разархивиране ... Грешка!'),PChar('Разархивиране'),MB_OK);
  Backuped:=True;
  OpenTables;
 end;
end;
...
if Key>#31 then
  ...
  begin
   if LastKey='?' then
    ...
    KeyBuff:=KeyBuff+Key;lastKey:=Key;
...
begin
   ...
   While (not CeniTable.Eof) do
     begin
       CeniTable.Edit;
       CeniTable.FieldValues['CENA']:=CeniTable.FieldValues['MINUTA']*solariumi.FieldValues['CENA'];
       CeniTable.Post;
       CeniTable.Next;
     end;
end;
...
var
 q    :TABSQuery;
begin
if (PasswordForm.ModalResult=MROK) or (PasswordForm.ShowModal=MROK) then begin
   q              :=TABSQuery.Create(nil);
   q.Databasename :='sol1';
   q.ReadOnly     :=False;
   q.RequestLive  :=True;
   q.SQL.SetText(PChar('SELECT * FROM kartichip WHERE KLIENTNOMER = '+IntToStr(QKLIENTI.FieldValues['NOMER'])));
   q.open;
   if q.IsEmpty and  (QKLIENTI.FieldValues['NOMER']>0) then  begin
     VipDiscount                 :=0;
     q.Append;
