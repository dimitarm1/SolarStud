...
interface
uses Classes, Variants, SysUtils, Dialogs, Forms, Windows, DB, AbsMain,
AbsTypes;
...
var
    ...
    DBVer: integer;
const
    DataBaseName = '\SolarStudio1\data\Solarbas.abs';
const
    CurrentDBVer = 3;
label
    endprogram;
    ...
    var
        ...
        Ver: Variant;
    begin
        if (tables.IndexOf('PARAMS') < 0) and (tables.IndexOf('PARAMS') < 0)
            then
            result := 0
        else
        begin
            ...
            Q.Active := true;
            if not Q.Locate('KEY', VarArrayOf(['DBVer']), []) then
                result := 3
            else
            begin
    ...
    begin
        ...
        Q.SQL.Text := 'SELECT * FROM PARAMS';
        Q.Active := true;
        if not Q.Locate('KEY', 'DBVer', []) then
        begin
            Q.Append;
...
begin
    result := true;
    DBFileName := ExtractFileDrive(Paramstr(0)) + DataBaseName;
        // Извлича драйва от командните параметри и името на EXE-то
    DBOK1 := false;
    ...
    begin
        ...
        begin
            if
                Application.MessageBox(PChar('Базата данни не е открита. Да създам ли нова база?'), PChar(''), MB_YESNO) = 6 then
            begin // 6= yes, 7=no
            ...
            begin
                ...
                if not dialog.Execute or (dialog.Files.Count = 0) then
                begin
                ...
                begin
                    result := CopyFIle(PChar(dialog.Files[0]),
                        PChar(DBFileName), False);
                end;
            end;
        end;
        try
    ...
    ExecSQL('delete from KARTICHIP where not KLIENTNOMER >0');
        //Truncate bad records silently
    ...
    begin // Попълване на таблица цени ако е празна
        CENA1 := 0;
        _Q2.SQL.Text := 'select * from SOLARIUM ';
        _Q2.readonly := False;
        _Q2.InMemory := false;
        _Q2.Active := true;
        while not _Q2.Eof do
        begin
            for i2 := 0 to 30 do
            begin
                _Q.Append;
    ...
    begin // Попълване на таблица цени ако е празна
        ...
        begin
            ...
            for i2 := 0 to 30 do
            begin
                if (not _Q.Locate('SOLARIUM;MINUTA', VarArrayOf([i1 + 100, i2]),
                    [])) then
                begin
                    _Q.Append;
